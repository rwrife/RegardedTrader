...
import { MentionStore, WatchlistStore, type LLM, type WebSearch } from '@regardedtrader/core';
  ...
  it('validates a symbol, persists it, and returns cached on a second call', async () => {
    ...
    const { app } = createApp({
      ...
      initialConfig: {
        ...
        polling: { sentimentSources: { reddit: { enabled: true, weight: 1 }, stocktwits: { enabled: true, weight: 0.7 }, hn: { enabled: true, weight: 0.4 }, cnn: { enabled: true, weight: 1.2 }, 'google-news': { enabled: true, weight: 1.1 } } },
      },
